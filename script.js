...
const ADMIN_UPI = "6267635352@api";
...
function renderWalletView() {
    ...
    <div class="card">
        ...
        ${(data.user.transactions || []).map(t => `
            <div class="info-row" style="margin-top:10px;"><span>${t.type} (${t.date})</span><span style="color:${t.amount > 0 ? 'var(--green)' : 'var(--red)'}">${t.amount > 0 ? '+' : ''}₹${t.amount}</span></div>
...
function showAddMoneyModal() {
    modal(`
        ...
        <h2>Add Money to Wallet</h2><p class="muted">Scan QR or use UPI to pay</p>
        <div class="field" style="margin-top:12px;"><label>Amount (₹)</label><input id="addAmount" type="number" value="50" onchange="updateQR()"></div>
        <div class="qr-card"><p style="font-size:12px;color:var(--muted)">Scan using any UPI App</p>
            ...
            <div class="upi-apps"><a href="upi://pay?pa=${ADMIN_UPI}&pn=${encodeURIComponent(ADMIN_NAME)}&am=50&cu=INR" class="upi-btn">Pay via UPI App</a></div>
        </div>
        <div class="field"><label>Transaction ID / UTR Number</label><input id="utrNo" placeholder="Enter 12-digit UTR No. after payment"></div>
...
function submitAddMoney() {
    ...
    save(); closeModal(); render(); toast(`₹${amt} added to wallet successfully!`);
}
...
function renderMyGamesView() {
    ...
    if (joinedMatches.length === 0) return `<div class="card"><p class="muted">Aapne abhi koi match join nahi kiya hai.</p></div>`;
    return joinedMatches.map(t => {
        const isRoomReady = t.roomId && t.roomPass;
        return `<div class="card"><h3>${t.name}</h3><p class="muted">⏰ Match Time: ${t.time} | Mode: ${t.mode}</p>
        ${isRoomReady ? `<div class="room-card"><p style="color:var(--green);font-weight:bold;font-size:13px;">✅ ROOM ID & PASSWORD RELEASED</p>
        <div class="room-row"><span>ROOM ID: <span class="room-val">${t.roomId}</span></span><button class="copy-btn" onclick="copyText('${t.roomId}')">COPY</button></div>
        <div class="room-row"><span>PASSWORD: <span class="room-val">${t.roomPass}</span></span><button class="copy-btn" onclick="copyText('${t.roomPass}')">COPY</button></div></div>` :
        `<div class="room-card" style="border-color:var(--line);"><p style="color:var(--gold);font-size:13px;">⏰ Room ID & Password match start hone se 15 min pehle dikhai dega.</p></div>`}</div>`;
    }).join('');
}
